...
contract ERC865Token is ERC865, StandardToken, Ownable {
    ...
    // from _signature we can know the sender's information
    ...
    function transferFromPreSigned(
        ...
        require(_to != address(0));
        require(signatures[_signature] == false);
        bytes32 hashedTx = transferFromPreSignedHashing(
            address(this),
            _from,
            _to,
            _value,
    ...
    function increaseApprovalPreSigned(
        ...
        require(_nonce == nonces[from].add(1));
        require(
            allowed[from][_spender].add(_addedValue).add(_fee) <= balances[from]
        );
    ...
    function decreaseApprovalPreSigned(
        ...
        nonces[from] = _nonce;
        signatures[_signature] = true;
        uint256 oldValue = allowed[from][_spender];
        if (_subtractedValue > oldValue) {
...
contract InvestmentToken is ERC865Token {
    using SafeMath for uint256;

    string public constant name = "Investment Token";
    string public constant symbol = "InvestmentToken";
    ...
    function get865FromContract(address _to, uint256 _amount)
        public
        returns (bool)
    {
