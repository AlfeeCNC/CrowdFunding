...
contract ERC865Token is ERC865, StandardToken, Ownable {
        ...
        returns (bytes32)
    {
        ...
        bytes32 output = keccak256(
          abi.encodePacked(bytes4(0xf7ac9c2e), _token, _spender, _value, _fee, _nonce)
        );
        return output;
    }
...
contract MoobaERC865Token is ERC865Token {
  using SafeMath for uint256;

  string public constant name = "ERC 865 Token";
  string public constant symbol = "865Token";
  ...
  // call this function when (core)company put margin or ask for loan (看要不要收手續費)
  // 這筆交易的手續費為平台支出
  function get865FromContract(address _to, uint256 _amount) public returns (bool) {
